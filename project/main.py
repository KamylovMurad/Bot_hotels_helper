...
def hello_world(message):
    bot.send_message(message.chat.id, f'● /lowprice — вывод самых дешёвых отелей в городе\n'
...
def price_hotels_cheap(message, *args):
    try:
        ...
        bot.reply_to(message, f'Введите количество отелей (цифрами) ')
        if args[0] == 'lowprice':
            bot.register_next_step_handler(message, date_start, region_id, *args)
...
def date_stop(message, *args):
    try:
        ...
        bot.reply_to(message, 'Введите планируемую дату выезда : (дд-мм-гггг)')
        bot.register_next_step_handler(message, calculation, *args, start)
    except TypeError:
        bot.reply_to(message, f'Неактульная дата заезда, укажите дату не ранее {date.today()}')
    except Exception:
...
def calculation(message, *args):
    try:
        ...
        try:
            ...
            bot.reply_to(message, f'Неактульная дата выезда, укажите дату не ранее {date.today()}')
    except Exception:
        bot.reply_to(message, f'Неверно указана дата выезда')


def price_range(message, *args):
    bot.reply_to(message, f'Введите диапазон цен через пробел в USD: "min max"  ')
...
def deal_func(message, *args):
    ...
    if len(price) == 2 and int(price[0]) < int(price[1]):
        ...
        list_hotels = search_hotels_by(region_id=region_id, minn=min_price, maxx=max_price, sort_type='DISTANCE')
        sort_price = bestdeal_price(list_hotels, maxx=max_price, minn=min_price)
        try:
            for sorted_hotels in range(0, maximum_hotels):
                hotel = search_distance_price(list_price=sort_price, hotel_list=list_hotels)
                hotel_name, hotel_id, hotel_price, destination = info_hotel(hotel)
                price_days = round(hotel_price * days, 2)
                search = search_hotel_info(hotel_id)
                location = get_location(search)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton('Получить фото', callback_data=f'hotel_{hotel_id}'))
                text = bot.send_message(message.from_user.id, f'Название : {hotel_name}\n'
            ...
            bot.send_message(message.from_user.id, 'Список отелей по данным параметрам закончился')
        finally:
            writing_history(str(message.from_user.id))
...
def hotels_max_high(message, *args):
    try:
        ...
        list_hotels = list(reversed(search_hotels_by(region_id=region, minn=450)))
        for elem in range(0, max_count):
            hotel_name, hotel_id, hotel_price, destination = info_hotel(list_hotels[elem])
            search = search_hotel_info(hotel_id)
            price_days = round(hotel_price * days, 2)
            location = get_location(search)
            markup = types.InlineKeyboardMarkup()
        ...
        bot.send_message(message.from_user.id, 'Список самых дорогих отелей в городе закончился')
    finally:
        writing_history(str(message.from_user.id))
...
def hotels_max_cheap(message, *args):
    ...
    for sorted_hotels in range(0, max_count):
        hotel_name, hotel_id, hotel_price, destination = info_hotel(list_hotels[sorted_hotels])
        search = search_hotel_info(hotel_id)
        price_days = round(hotel_price * days, 2)
        location = get_location(search)
        markup = types.InlineKeyboardMarkup()
